...
#  including suggestions/developments by users...]
    ...
    def plotfield(self, title=None, labels=None,
                  colorbar='vertical',
        ...
        if title == None:
            title = str(self.comment) + '\n' + \
                    str(self.validity.get())
        if len(self.validity) == 1:
            return plotprofiles(self, title=title, labels=labels)
        else:
...
def plotverticalhovmoller(self,
                  colorbar='vertical',
                  graphicmode='colorshades',
                  minmax=None,
                  levelsnumber=21,
        ...
        f = plt.figure()
        # coords
        z = numpy.zeros((len(self.validity),
        ...
        x = numpy.zeros((len(self.validity),
                         len(self.geometry.vcoordinate.levels)))
        validities = {self.validity[i].get():i for i in range(len(self.validity))}
        date = 'Validity'
        if len(validities) == 1 and len(self.validity) != 1:
            date = 'Basis'
            validities = {self.validity[i].getbasis():i for i in len(self.validity)}
        epoch = datetime.datetime(1970, 1, 1)

        for i in range(len(self.validity)):
            d = self.validity[i].get() if date == 'Validity' else self.validity[i].getbasis()
            timedelta = d - epoch
        ...
        if title is None:
            if fidkey is None:
                ...
                fid = self.fid[fidkey]
            title = 'Vertical Hovmöller of ' + str(fid)
        f.axes[0].set_title(title)

        return f

def plotprofiles(profiles, labels=None, fidkey=None, Ycoordinate=None,
    ...
    i = 0
    for p in profiles:
        if p.geometry.vcoordinate.typeoffirstfixedsurface == 100:
            Y = numpy.array(p.geometry.vcoordinate.levels)  # / 100
        elif p.geometry.vcoordinate.typeoffirstfixedsurface in (102, 103, 109):
            Y = numpy.array(p.geometry.vcoordinate.levels)
        if labels != None:
            label = labels[i]
        else:
            if fidkey != None:
