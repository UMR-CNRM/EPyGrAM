...
#  including suggestions/developments by users...]
    ...
    def plotfield_Magics(self,
        ...
        zmin = self.data.min()
        zint = (self.data.max() - self.data.min()) / 20.
        magcont = Magics.macro.mcont(legend='on', contour='off', contour_shade='on', contour_label='off',
                                     contour_shade_method='area_fill', contour_min_level=zmin, contour_shade_min_level=zmin,
                                     contour_level_selection_type='interval', contour_interval=zint,
                                     contour_shade_min_level_colour='blue', contour_shade_max_level_colour='red',
                                     contour_shade_colour_direction='clockwise', contour_reference_level=zmin)
        magpage = Magics.macro.page(super_page_x_length=21.,super_page_y_length=21.,
                                    page_x_length=21.,page_y_length=21.,page_id_line='off')
        magcoast = Magics.macro.mcoast(map_coastline_thickness=0,
                                       map_boundaries='off', map_boundaries_colour='black', map_boundaries_thickness=2,
                                       map_administrative_boundaries='off', map_administrative_boundaries_countries_list=['FRA'],
                                       map_administrative_boundaries_colour='black', map_administrative_boundaries_thickness=3,
                                       map_grid_thickness=1, map_grid_latitude_increment=1., map_grid_longitude_increment=1.,
                                       map_label_height=1.)
...
#        slat = lat.min()
#        nlat = lat.max()
#        magmap = Magics.macro.mmap(subpage_map_projection='polar_stereographic',
#                                   subpage_map_vertical_longitude= (wlon+elon)/2. ,
#                                   #map_projection='cylindrical',map_area_definition='corners',
#                                   subpage_lower_left_latitude=float(slat),
#                                   subpage_lower_left_longitude=float(wlon),
#                                   subpage_upper_right_latitude=float(nlat),
#                                   subpage_upper_right_longitude=float(elon)  )

        corners = self.geometry.gimme_corners_ll()
        magmap = Magics.macro.mmap(subpage_map_projection='lambert', subpage_map_area_definition='corners',
                                   subpage_lower_left_latitude=float(corners['ll'][1]),
                                   subpage_lower_left_longitude=float(corners['ll'][0]),
                                   subpage_upper_right_latitude=float(corners['ur'][1]),
                                   subpage_upper_right_longitude=float(corners['ur'][0])  )
        magleg = Magics.macro.mlegend(legend_display_type='continuous',
                                      legend_entry_plot_direction='column', legend_entry_plot_orientation='top_bottom',
                                      legend_text_colour='black', legend_text_format='(F4.0)', legend_text_font_size=1.,
                                      #legend_title='on', legend_title_text=cname, legend_title_position='top',
                                      legend_title_position_ratio=10., legend_box_mode='positional',
                                      legend_box_x_position=18., legend_box_y_position=1.,
                                      legend_box_x_length=4., legend_box_y_length=16., legend_border='off')
        magtext = Magics.macro.mtext(text_font_size=1.5,text_colour='black',
                                     text_line_1=title )
        defout={'output_formats':['png'], 'output_fullname':plotfile,
                'output_name_first_page_number':'off',
                'output_width':1400} #This item for png output
        magout = Magics.macro.output(**defout)

        #GRIBField = GRIBmessage(('field', self))
        #fld = Magics.macro.mgrib(grib_id=GRIBField._gid)
        
        #We must:
        # - use 'packingType':'grid_simple' to prevent "numberOfPoints != size(values) (481401!=3600)" error
        # - not use grib_edition=1 to prevent "gribapi.GribInternalError: Value is read only" error when doing "self['LaDInDegrees'] = lat_0" in line 427 of _build_msg_from_field
        
        #pb indentation dans GRIBmessage.update: on ne fait rien avec E si c'est un dict 
        
        import epygram
        r = epygram.formats.resource("/tmp/titi.grb", "w", "GRIB")
        ordering = {'iScansNegatively':0, 'jScansPositively':1}
        r.writefield(self, grib_edition=2, packing={'packingType':'grid_simple'}, ordering=ordering)
        r.close()
        fld = Magics.macro.mgrib(grib_input_file_name="/tmp/titi.grb", grib_automatic_scaling='off',grib_field_position=1)
        
        #lon, lat = self.geometry.get_lonlat_grid()
        #fld = Magics.macro.minput(input_type = "geographical",
        #                          input_values = self.data.flatten(),
        #                          input_latitude_values = lat.flatten(),
        #                          input_longitude_values = lon.flatten(),
        #                          input_binning = "on", #It would be simpler to use "off" but don't seem to work
        #                          binning_x_method = "count",
        #                          binning_x_count = 60,
        #                          binning_y_method = "count",
        #                          binning_y_count = 60)
        
        Magics.macro.plot(magout, magpage, magmap, fld, magcont, magcoast, magleg, magtext)
